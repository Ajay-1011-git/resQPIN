...
///
/// Instead of a single circle per state, crime totals are distributed
/// across major cities/districts within each state for a realistic,
/// geographically spread visualization.
///
...
class _HeatmapScreenState extends State<HeatmapScreen> {
  // ═══════════════════════════════════════════════════════════════════════════
  // Distributed crime data — major cities within each state
  // Crime counts are proportionally distributed from state totals
  // based on approximate urban population ratios.
  // ═══════════════════════════════════════════════════════════════════════════
  static const List<_CityData> _distributedData = [
    // ─── MADHYA PRADESH (Total: 194,711) ──────────────────────────
  ...
  static Color _crimeColor(int crimes) {
    if (crimes >= 40000) return const Color(0xCCFF1744); // Critical
    if (crimes >= 25000) return const Color(0xCCFF6D00); // Very High
    if (crimes >= 15000) return const Color(0xCCFFAB00); // High
    if (crimes >= 8000) return const Color(0xCCFFFF00); // Moderate
    if (crimes >= 3000) return const Color(0xCC76FF03); // Low
    return const Color(0xCC00E676); // Very Low
  }
          ...
          Container(
            ...
            child: Column(
              ...
              children: [
                const Row(
                  children: [
                    Icon(Icons.auto_graph, size: 14, color: Colors.white70),
                    SizedBox(width: 6),
                    Expanded(
                      child: Text(
